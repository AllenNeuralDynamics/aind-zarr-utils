...
class TestPoints:
    ...
    def test_constructor_coerces_to_float(self) -> None:
        pts = Points(values={"a": np.array([[1, 2, 3]])}, space=Space.ZARR_INDICES)
        assert pts.values["a"].dtype.kind == "f"
        np.testing.assert_allclose(pts.values["a"], [[1.0, 2.0, 3.0]])

    def test_constructor_rejects_invalid_shape(self) -> None:
        with pytest.raises(ValueError, match="Expected"):
            Points(values={"a": np.array([1.0, 2.0, 3.0])}, space=Space.ZARR_INDICES)

    def test_constructor_validates_description_lengths(self) -> None:
...
class TestFromSWC:
    ...
    def test_fractional_swc_values_are_not_rounded(self) -> None:
        arr = np.array([[1.25, 2.5, 3.75]])
        pts = Points.from_swc(arr, units="millimeter")
        np.testing.assert_allclose(pts.values["_"], [[1.25, 2.5, 3.75]])

    def test_invalid_axis_order_raises(self) -> None:
        with pytest.raises(ValueError, match="axis_order"):
            Points.from_swc(np.array([[1.0, 2.0, 3.0]]), axis_order="zzz", units="millimeter")
...
def _make_asset_for_real_zarr(
    ...
    *,
    ...
    return Asset(alignment_zarr_uri=real_ome_zarr, metadata=metadata, processing=processing)
...
def test_neuroglancer_round_trip_uses_transform_method(
    ...
    processing = create_comprehensive_processing_data()
    asset = Asset(alignment_zarr_uri=real_ome_zarr, metadata=mock_nd_metadata, processing=processing)
    pts = Points.from_neuroglancer(neuroglancer_test_data)
    ccf = asset.transform(pts, to=Space.CCF_MM)
...
def test_transform_points_freestanding_function(
    ...
    asset = Asset(alignment_zarr_uri=real_ome_zarr, metadata=mock_nd_metadata, processing=mock_processing_data)
