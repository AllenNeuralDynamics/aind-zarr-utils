...
def _make_explicit_asset(alignment_zarr_uri: str = "s3://bucket/data/asset/image.zarr/") -> Asset:
    ...
    return Asset(alignment_zarr_uri=alignment_zarr_uri, metadata=metadata, processing=processing)
...
class TestAssetExplicit:
    ...
    def test_explicit_construction_no_io(self) -> None:
        ...
        with (
            ...
            assert asset.alignment_zarr_uri == "s3://bucket/data/asset/image.zarr/"
            assert asset.source_zarr_uri is None
            mock_open.assert_not_called()
            mock_align.assert_not_called()
            mock_xforms.assert_not_called()

    def test_asset_is_frozen(self) -> None:
        """Asset instances do not allow public attribute reassignment."""
        asset = _make_explicit_asset()
        with pytest.raises(AttributeError):
            asset.alignment_zarr_uri = "s3://bucket/other.zarr"  # type: ignore[misc]
    ...
    def test_opened_zarr_lazy_and_cached(self) -> None:
        ...
        with patch("aind_zarr_utils.asset._open_zarr", return_value=sentinel) as mock_open:
            ...
            mock_open.assert_called_once_with(asset.alignment_zarr_uri)
...
class TestAssetEagerConstructors:
    ...
    def test_from_root_eager_open(self) -> None:
        ...
        with (
            ...
            assert asset.alignment_zarr_uri == zarr_uri
            assert asset.source_zarr_uri is None
            assert dict(asset.metadata) == metadata
            assert dict(asset.processing) == processing
            # Subsequent .opened_zarr access does NOT re-open.
            assert asset.opened_zarr is sentinel
            mock_open.assert_called_once()

    def test_from_zarr_records_source_uri(self) -> None:
        """from_zarr keeps the input Zarr URI separate from the resolved alignment URI."""
        source_uri = "s3://bucket/data/asset/image_tile_fusing/OMEZarr/other.zarr"
        alignment_uri = "s3://bucket/data/asset/image_tile_fusing/OMEZarr/Ex.zarr"
        ...
        with (
            patch(
                "aind_zarr_utils.asset.alignment_zarr_uri_and_metadata_from_zarr_or_asset_pathlike",
                return_value=(alignment_uri, metadata, processing),
            ),
            patch("aind_zarr_utils.asset._open_zarr", return_value=("n", {})),
        ):
            asset = Asset.from_zarr(source_uri)

            assert asset.alignment_zarr_uri == alignment_uri
            assert asset.source_zarr_uri == source_uri
    ...
    def test_from_neuroglancer_uses_first_image_source(self) -> None:
        ...
        with (
            ...
            assert asset.alignment_zarr_uri == zarr_uri
            assert asset.source_zarr_uri == "s3://bucket/x.zarr"
...
@pytest.mark.parametrize("method", ["image", "stub"])
def test_origin_rejected_when_pipeline_true(method: str) -> None:
    """Pipeline builders own their corrected origin and reject caller origin."""
    asset = _make_explicit_asset()
    with pytest.raises(ValueError, match="origin cannot be used"):
        getattr(asset, method)(pipeline=True, origin=Origin.at((1.0, 2.0, 3.0)))
