...
- New asset-centric API. The package's recommended entry point is now
  ...
  `Asset.from_neuroglancer`, plus a no-I/O
  `Asset(alignment_zarr_uri, metadata, processing)` constructor). A
  single `Asset` instance opens its alignment-channel Zarr exactly once
  and resolves transform-chain paths exactly once, regardless of how many
  methods participate in a workflow. Assets constructed from another
  acquisition Zarr also retain that input as `source_zarr_uri`.
...
- `Points.from_neuroglancer` and `Points.from_swc` provide one-line
  ...
  canonical representation. `Points` validates `(N, 3)` arrays and
  coerces them to float. The new SWC path preserves continuous
  sub-voxel coordinates; the legacy SWC helper still rounds to integer
  Zarr indices.
