...
class Asset:
    ...
    source_zarr_uri : str, optional
        Original acquisition Zarr URI used for discovery. Set by
        :meth:`from_zarr` and :meth:`from_neuroglancer`; ``None`` for
        :meth:`from_root` and explicit construction unless supplied.
    ...
    def __post_init__(self) -> None:
        """Detach top-level metadata mappings from caller-owned dictionaries."""
        object.__setattr__(self, "metadata", MappingProxyType(dict(self.metadata)))
        object.__setattr__(self, "processing", MappingProxyType(dict(self.processing)))
    ...
    def _eagerly(
        ...
        *,
        ...
        asset = cls(**kwargs)
        object.__setattr__(asset, "_opened_zarr", _open_zarr(zarr_uri))
        return asset

    # --------------------------------------------------------------- accessors

    def _metadata_dict(self) -> dict[str, Any]:
        """Return metadata for legacy helpers that are typed as accepting dicts."""
        return cast(dict[str, Any], self.metadata)

    def _processing_dict(self) -> dict[str, Any]:
        """Return processing metadata for legacy helpers typed as accepting dicts."""
        return cast(dict[str, Any], self.processing)
    ...
    def opened_zarr(self) -> tuple[Node, dict]:
        ...
        if self._opened_zarr is None:
            object.__setattr__(self, "_opened_zarr", _open_zarr(self.alignment_zarr_uri))
        opened = self._opened_zarr
    ...
    def image(
        ...
        *,
        ...
        origin : Origin, optional
            ...
            Only valid when ``pipeline=False``; pipeline images use the
            corrected header's origin.
        ...
        sitk.Image or ants.core.ANTsImage
            ...
            ``pipeline=True``.
        """
        if pipeline and origin is not None:
            raise ValueError("origin cannot be used when pipeline=True")
    ...
    def stub(
        ...
        *,
        ...
        existing ``zarr_to_sitk_stub`` API). ``origin`` is only valid when
        ``pipeline=False``; pipeline stubs use the corrected header's origin.
        """
        if pipeline and origin is not None:
            raise ValueError("origin cannot be used when pipeline=True")
        if pipeline:
