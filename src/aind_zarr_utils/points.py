...
class Points:
    ...
    def __post_init__(self) -> None:
        """Validate and normalize point arrays."""
        clean_values: dict[str, NDArray] = {}
        for layer, arr in self.values.items():
            arr_float = np.asarray(arr, dtype=float)
            if arr_float.ndim != 2 or arr_float.shape[1] != 3:
                raise ValueError(f"Expected (N, 3) array for layer {layer}, got shape {arr_float.shape}")
            clean_values[layer] = arr_float

        clean_descriptions: dict[str, list[str | None]] | None = None
        if self.descriptions is not None:
            clean_descriptions = {}
            for layer, desc in self.descriptions.items():
                if layer not in clean_values:
                    raise ValueError(f"Descriptions contain unknown layer {layer!r}")
                desc_list = list(desc)
                if len(desc_list) != len(clean_values[layer]):
                    raise ValueError(
                        f"Descriptions for layer {layer!r} have length {len(desc_list)}, "
                        f"expected {len(clean_values[layer])}"
                    )
                clean_descriptions[layer] = desc_list

        object.__setattr__(self, "values", clean_values)
        object.__setattr__(self, "descriptions", clean_descriptions)
    ...
    def from_swc(
        ...
        *,
        ...
        if sorted(order_lower) != ["x", "y", "z"]:
            raise ValueError("axis_order must be a permutation of 'zyx'")
...
def _indices_to_base_anat(asset: Asset, pts: Points) -> Points:
    ...
    out = annotation_indices_to_anatomical(base_stub, dict(pts.values))
...
def _indices_to_pipeline_anat(asset: Asset, pts: Points) -> Points:
    ...
    out = annotation_indices_to_anatomical(pipeline_stub, dict(pts.values))
    return Points(
        values=out,
